...
GameStateHitGoalBox::GameStateHitGoalBox(Game *pGame)
	: GameStateBase(pGame)
{
	m_nType = GAMESTATE_HITGOALBOX;
	m_nTimeStamp = -1;
}

GameStateHitGoalBox::~GameStateHitGoalBox()
{
}

void GameStateHitGoalBox::Init()
{
	DeleteOldState();
	...
	SOUND_MANAGER->PlayBGM(L"HitGoalBox", false);
}

void GameStateHitGoalBox::Process()
{
	if (m_pGame->m_nStagePackType == STAGEPACK_SINGLE)
	{
		if (m_nStateFrame == 220)
		{
		...
		if (m_nStateFrame > 220 && m_nTimeStamp != -1)
		{
			if (m_pGame->m_nStagePackType == STAGEPACK_SINGLE)
...
void GameStateHitGoalBox::Render()
{
