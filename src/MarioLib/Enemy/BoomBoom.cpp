...
BEGIN_IMPL_PROPERTYMAP(BoomBoom)
	PROP_BOOL("PowerUp", VT_BOOL, false, nullptr, "BoomBoom"),
	PROP_BOOL("Winged", VT_BOOL, false, nullptr, "BoomBoom")
END_IMPL_PROPERTYMAP()

BoomBoom::BoomBoom(Game *pGame, Stage *pStage) :
	EnemyObjectBase(pGame, pStage)
{
	m_nType = ENEMY_BOOMBOOM;
	m_nState = STATE_IDLE;
...
void BoomBoom::ChangeState(int nState, ChangeStateInfo *pInfo)
{
	switch (nState)
	{
	case STATE_PRESSED:		
		{
			...
			default:
				...
				else
				{
					if (m_bPowerUp)
					{
						// Create normal size BoomBoom x2
						SplitToTwoBoomBooms();
						nState = STATE_DEAD;
					}
				}
...
void BoomBoom::OnChangeState(int nState)
{
...
void BoomBoom::OnPressed()
{
	if (m_nStateFrame > 40)
		ChangeState(STATE_DEAD);
}

bool BoomBoom::IsAlive()
{
	return EnemyObjectBase::IsAlive();
}

void BoomBoom::SplitToTwoBoomBooms()
{
	GameObjectBase *pObj;
...
std::vector<POINT> BoomBoom::GetWingPosition()
{
	POINT pt;
	SIZE s = GetSize();
	pt.x = -8;
...
int BoomBoom::GetWingSpriteIndex()
{
