...
// 	{
// 		NaDebugOut(L"(OPM) Property: %s, 0x%08x\n", it->first.wstr());
// 	}
// 	NaDebugOut(L"(OPM) -----------------------\n");
#endif
}
...
void ObjectPropertyMapper::RefreshControl()
{
	...
	//NaDebugOut(L"(OPM) RefreshControl begin\n");
	...
	for (int i = 0; i < nCnt; i++)
	{
		...
		for (int j = 0; j < nSubCnt; j++)
		{
			...
			if (pSubProp)
			{
				//NaString strName = pSubProp->GetName();
				//NaDebugOut(L"(OPM) RefreshControl: %s\n", strName.wstr());
	...
	//NaDebugOut(L"(OPM) RefreshControl end\n");
}

void ObjectPropertyMapper::SetControlValue(NaString strName, VARIANT * pVar)
{
	if (m_pCtrl == nullptr)
		return;
	...
	for (int i = 0; i < nCnt; i++)
	{
		...
		for (int j = 0; j < nSubCnt; j++)
		{
			...
			if (pSubProp)
			{
				NaString strControlName = pSubProp->GetName();
				if (strName.Compare(strControlName) == 0)
				{
					//DWORD dwData = pSubProp->GetData();
					//VARIANT var = GetPropertyByID(dwData);

					COleVariant var2(pVar);
					pSubProp->SetValue(var2);
				}
			}
		}
